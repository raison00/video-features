...
function cmCreateTechPropsTag(cm_ven, cm_cat, cm_pla, cm_ite, linkShareID, custID, refURL, attributes) {
    var pageID = CEconfig.coremetricsValue.pageId || null,
        categoryID = CEconfig.coremetricsValue.categoryID || null,
...
function cmCreateProductviewTag(productID, productName, cm_ven, cm_cat, cm_pla, cm_ite, linkShareID, custID, refURL, cmCrossSell, totalReviewCount, avgRating, numberRatingsOnlyReviews, buyAgainPercentage, attributes) {
    var categoryID = ((CEconfig.coremetricsValue===undefined)?null:CEconfig.coremetricsValue.categoryID)|| null,
         pageType = ((CEconfig.coremetricsValue===undefined)?null:CEconfig.coremetricsValue.pageType) || null;
    categoryID = findTypeOfPage(pageType) + categoryID;
    if (attributes) {
        var cm_exAttr = new Array;
...
function cmCreateBazaarViewTag(productID, productName) {
    var categoryID = ((CEconfig.coremetricsValue===undefined)?null:CEconfig.coremetricsValue.categoryID)|| null,
         pageType = ((CEconfig.coremetricsValue===undefined)?null:CEconfig.coremetricsValue.pageType) || null;
    categoryID = findTypeOfPage(pageType) + categoryID;
...
function cmCreateShopAction5Tag(productID, productName, productQuantity, productPrice, MasterProductID, MasterProductName, MasterCatID, IsMaster, cmCrossSell, attributes) {
    var categoryID = ((CEconfig.coremetricsValue===undefined)?null:CEconfig.coremetricsValue.categoryID)|| null,
         pageType = ((CEconfig.coremetricsValue===undefined)?null:CEconfig.coremetricsValue.pageType) || null;
    categoryID = findTypeOfPage(pageType) + categoryID;
...
function cmCreateShopAction9Tag(productID, productName, productQuantity,
    ...
    orderTotal, MasterProductID, MasterProductName,
    ...
    var categoryID = CEconfig.coremetricsValue.categoryID || null,
    pageType = CEconfig.coremetricsValue.pageType || null;
    categoryID = findTypeOfPage(pageType) + categoryID;
    if (attributes) {
        __ex = attributes.split("-_-");
...
function cmCreateUserErrorTag(mMsgCode, mMsgType, mMsgClass, mMsgDesc, mServerName, mCloneName) {
    var pageID = CEconfig.coremetricsValue.pageId || null,
        categoryID = CEconfig.coremetricsValue.categoryID || null,
        pageType = CEconfig.coremetricsValue.pageType || null;
    if (pageID == null) {
...
function cmCreateDelayedShopTag(productID, productName, productQuantity, productPrice, cmReason, cmShipDays) {
    var categoryID = CEconfig.coremetricsValue.categoryID || null,
        pageType = CEconfig.coremetricsValue.pageType || null;
    categoryID = findTypeOfPage(pageType) + categoryID;
...
function cmCreateLinkClickTag(obj,name,pageId,href, hostingUrl, referingUrl) {
    ...
    if(href===undefined){
      href=obj.href+"?cm_sp="+CEconfig.coremetricsValue.pageId+"-_-"+CEconfig.coremetricsValue.pageType+"-_-"+name;
    }
    pageID = findTypeOfPage(pageType) + pageID+"-"+name;
    
    cmMakeTag(["tid", "8", "pi", pageID, "ti", name, "hr", href, "ul", hostingUrl, "rf", referingUrl,"cm_sp","cmsp"]);
}
